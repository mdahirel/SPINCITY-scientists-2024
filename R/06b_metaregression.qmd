...
It can be useful to check if our city-level moderators are correlated. In addition, we are adding to the correlation matrix two more variables: precipitation, to confirm that it is correlated to temperature in our sample (explaining why we only use one of the two), and latitude, because of its potential link with many variables

```{r moderators-correlation}
data |> 
  group_by(urban_area) |> 
  summarise(
    tmp=mean(annual_tmp),
    logarea=mean(log(area_LAND_km2,10)),
    human_modif=mean(mean_human_modification_50km),
    pTREE=mean(mean_pTREE),
    yday=mean(mean_yday_prop), # we take the average city-level value for cities visited multiple times
    latitude=mean(lat),
    pre=mean(annual_pre)
    )  |> 
  ungroup() |> 
...
make_main_plot <- function(model, data, resp) {
  ...
  if (resp == "log_abdomen") {
    ...
    p_main <- ggplot() +
      ...
      facet_wrap(~"relative abdomen area") +
      theme(legend.position = "none", strip.text = element_text(size=11))
  }
