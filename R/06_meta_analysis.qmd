...
ggplot(data2) +
  ...
  scale_y_continuous("Effect size (Zr)") +
  scale_x_continuous("Annual mean temperature (°C)") + 
...
Based on Senior et al 2016 (REF), we'd expect typical heterogeneity % to be in the high range for ecological data (the paper mention an average around 90% in the range of meta-analyses they checked). We can do a bit of prior predictive check, using the sampling variances for each effect size:
...
Note that because we have multilevel models, we need to account for that when doing prior checks to chose the SDs
```{r}
set.seed(42)
priorSD = 0.4
...
a $\mathrm{HalfNormal}(0,0.4)$ on random effects SD would seem to behave well for both. A prior SD slightly smaller or larger (0.3 or 0.5) would likely work too, but in the latter the implied prior around heterogeneity seems overly narrow.
For the fixed effect priors, we follow the suggestion in Röver et al (REF) that the scale of the expected prior heterogeneity (so total RE variance) is a good indicator.
...
plot_annotations <- tibble(
  ...
  full_names = c("relative abdomen area","spider length",
               ...
               "abdomen brightness"),
  negative_annotation=c("smaller \n in cities","smaller \n in cities",
                             "smaller webs \n in cities","tighter meshes\n in cities",
                        "darker \n in cities"),
       positive_annotation=c("larger \n in cities","larger \n in cities",
...
```{r}
overall_fits <- tibble(overall_fits=list(
...
varcomps <- varcomps |> 
  ...
  mutate(I2 = var_rand/(var_rand + var_res),
...
make_caterpillar_plot<- function(ES_fits,PI_fits,overall_fits,ES_observed,plot_annotations,response){
 ...
 ranking <- ES_fits|> filter(response_name==response)|> group_by(study) |> 
   summarise(mean=mean(.epred)) |> mutate(ranked=factor(rank(mean))) 
 
p_main <- ggplot() +
  stat_lineribbon(data = expand_grid(filtered_PI,ranking),
                  aes(x = overall.PI,y=ranked), .width = 0.95, fill = pale_colour) +
  stat_lineribbon(data = expand_grid(filtered_overall,ranking),
...
fig3 <- (plot_size+plot_abdomen+plot_radius+plot_mesh+plot_brightness)+
  plot_layout(ncol=3,nrow=2)
```

```{r, dev='cairo_pdf'}
ggsave(here("plots","fig3.pdf"), plot = fig3, width = 9, height = 6)
ggsave(here("plots","fig3.png"), plot = fig3, width = 9, height = 6)
```
...
make_forest_plot <- function(ES_fits,PI_fits,overall_fits,ES_observed,plot_annotations,response){
  ...
  p_main <- ggplot() +
  stat_lineribbon(data = expand_grid(PI_fits|> filter(response_name==response),study=study_list),
                  aes(x = overall.PI,y=study), .width = 0.95, fill = pale_colour) +
  stat_lineribbon(data = expand_grid(overall_fits|> filter(response_name==response),study=study_list),
...
arrows_annotate <- plot_annotations |>
  filter(response_name==response) |> 
  pivot_longer(cols=c(negative_annotation,positive_annotation),
               values_to = "label")|> 
  mutate(x = c(-0.4, 0.4)) |>
  ggplot() +
