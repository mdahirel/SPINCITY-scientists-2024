...
We then create columns with the coordinates of the individual records from degrees to m, for use in spatial autocorrelation diagnostics later on. At the same time, we also make a column about whether the record was made inside the focal urban area sensu GHSL, or outside (while you can have valid urbanization gradients using only records made inside a UC/DUC, especially given these are defined quite coarsely, this is still a useful information to have)
...
records_filtered_mollweide <- records_filtered |>
  mutate(
    ...
    is_in_UC_DUC = ID %in% st_intersection(x=coords_m,y=polygons)$ID # some warning is expected
  )
```
...
step6 <- step5 |>
  mutate(
    ...
    p_in_UC_DUC = map(.x = complete_obs, .f = function(.x) {
      mean(as.numeric(.x$is_in_UC_DUC))
    })
  )
```
...
step12 <- step11 |>
  select(
    ...
    response_name, 
    urban_scale=scale, 
    p_in_UC_DUC,mean_yday,mean_yday_prop,
  ...
  unnest(c(ES_baseline,ES_spatial, 
    AIC_baseline,AICc_baseline,AIC_spatial,p_in_UC_DUC,mean_yday, mean_yday_prop)) |>
  ungroup() |> 
