---
title: "Summarising CRU-TS climatologies of GHSL urban areas - precipitation"
format: html
editor_options: 
...
```{r cru-tbl}
cru_tbl <- st_extract(cru["pre"], at = coords) |> 
  # some issues with ancillary variables here but absent from tmp, so extract only pre here
  as_tibble() |>
  mutate(
    year = year(time),
...
tab <- cru_tbl |>
  ...
  select(c(pre, month, year, urban_area)) |>
  group_by(urban_area, year) |>
  nest(data = c(pre, month)) |>
  mutate(
    annual_pre = map(
      .x = data,
      .f = function(.x) {
        annual_pre <- sum(.x$pre)
      }
    )
  ) |>
  unnest(annual_pre) |>
...
tab_avg <- tab |>
  ...
  summarise(annual_pre = mean(annual_pre)) |>
  ungroup()
```
...
```{r save-CRU}
write_csv(tab_avg, here("data", paste0("CRUpre_", tab_avg$year_range_CRU[1], ".csv")))
```
